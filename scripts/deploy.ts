...
async function main() {
  const environmet: string | undefined = process.env.ENV;
  if (!['ROPSTEN', 'MAINNET'].includes(environmet as string)) {
    throw Error(`Invalid environment: ${environmet}`);
  }

  const signer = await ethers.getSigners();

  const isRopsten: boolean = environmet === 'ROPSTEN';
